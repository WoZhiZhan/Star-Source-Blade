...
public class SparkSlashArts extends SlashArts {
    public SparkSlashArts(Function<LivingEntity, ResourceLocation> state) {
        super(state);
    }
    ...
    public ResourceLocation doArts(ArtsType type, LivingEntity user) {
        ...
        stateOpt.ifPresent(state -> {
            ...
            if (user instanceof Player player) {
                EntitySuperDimension superDimension = new EntitySuperDimension(ModEntities.SUPER_DIMENSION.get(), world, player, cost);
                superDimension.setPos(player.getX(), player.getY(), player.getZ());
                superDimension.setLifeTime(200);
                world.addFreshEntity(superDimension);
            }
        });
