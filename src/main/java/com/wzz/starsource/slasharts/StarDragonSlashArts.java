...
public class StarDragonSlashArts extends SlashArts {
    public StarDragonSlashArts(Function<LivingEntity, ResourceLocation> state) {
        super(state);
    }
            ...
            if (user instanceof Player player) {
                ...
                for (int i = 0; i < count; i++) {
                    if (!world.isClientSide()) {
                        StormSwordSpawner.executeIceSwordWave(player, world, 1, 10);
                        StormSwordSpawner.executeStormSwordAttack(player, world, 1, 10);
                        StormSwordSpawner.executeStormSwordAttack(player, world, 2, 10);
                    }
                }
                for (Entity entity : ModUtils.findEntitiesAlongRay(player, 7, 2.5d)) {
                    if (entity instanceof LivingEntity living && living != player) {
                        living.setDeltaMovement(0,0.5,0);
                        living.hurt(living.damageSources().playerAttack(player), 50f);
                    }
                }
                ModUtils.moveTo(player, 3);
            }
        });
