...
public class SuperAstralShadowSlashArts extends SlashArts {
    public SuperAstralShadowSlashArts(Function<LivingEntity, ResourceLocation> state) {
        super(state);
    }
            ...
            if (user instanceof Player player) {
                ...
                for (int i = 0; i < count; i++) {
                    if (!world.isClientSide()) {
                        StormSwordSpawner.executeStormSwordAttack(player, world, 1, 30);
                    }
                }
            }
        });
