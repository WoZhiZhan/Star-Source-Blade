...
public class UltimateAstralShadowSlashArts extends SlashArts {
    public UltimateAstralShadowSlashArts(Function<LivingEntity, ResourceLocation> state) {
        super(state);
    }
            ...
            if (user instanceof Player player) {
                ...
                for (int i = 0; i < count; i++) {
                    if (!world.isClientSide()) {
                        StormSwordSpawner.executeStormSwordAttack(player, world, 2, 30);
                    }
                }
            }
        });
