...
import static net.wzz.starsource.item.ItemAnnihilateSlashBlade.scale;

public class EntityAnnihilationDimension extends Entity implements IThrowableEntity {
    ...
    public EntityAnnihilationDimension(World par1World) {
        super(par1World);
    ...
    public EntityAnnihilationDimension(World par1World, EntityLivingBase entityLiving, float AttackLevel, boolean multiHit) {
        this(par1World, entityLiving, AttackLevel);
        this.setIsSingleHit(multiHit);
    }

    public EntityAnnihilationDimension(World par1World, EntityLivingBase entityLiving, float AttackLevel) {
        this(par1World);
    ...
    public void onUpdate() {
        ...
        if (ItemAnnihilateSlashBlade.boom || scale <= 0.0001f) {
            for (Entity e : world.getEntitiesWithinAABBExcludingEntity(null,
                    getEntityBoundingBox().grow(40))) {
                if (e != null && e != this && !(e instanceof EntityPlayer) && !(e instanceof EntityColorLightningBolt)) {
                    ...
                    e.attackEntityFrom(DamageSource.OUT_OF_WORLD, 150);
                    this.setDead();
                    this.onRemovedFromWorld();
                    ItemAnnihilateSlashBlade.boom = false;
                }
            }
        }
        if (scale <= 0.0001f)
    ...
    static {
        LIFETIME = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.VARINT);
        SINGLE_HIT = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.BOOLEAN);
        IS_SLASH_DIMENSION = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.BOOLEAN);
        THROWER_ENTITY_ID = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.VARINT);
        INTERVAL = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.VARINT);
        COLOR = EntityDataManager.createKey(EntityAnnihilationDimension.class, DataSerializers.VARINT);
    }
}
