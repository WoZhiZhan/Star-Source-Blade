...
public class EntitySuperDimension extends Entity implements IThrowableEntity {
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            for (Entity e : world.getEntitiesWithinAABBExcludingEntity(null,
                    getEntityBoundingBox().grow(10))) {
                if (e != null && e != this && !(e instanceof EntityPlayer)) {
                    e.setFire(10);
                    e.attackEntityFrom(DamageSource.OUT_OF_WORLD,6);
    ...
    static {
        LIFETIME = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.VARINT);
        SINGLE_HIT = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.BOOLEAN);
        IS_SLASH_DIMENSION = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.BOOLEAN);
        THROWER_ENTITY_ID = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.VARINT);
        INTERVAL = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.VARINT);
        COLOR = EntityDataManager.createKey(EntitySuperDimension.class, DataSerializers.VARINT);
    }
}
