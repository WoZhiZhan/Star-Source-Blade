...
import net.wzz.starsource.load.specialAttacks.JudgeStarSpecialAttacks;

import javax.annotation.Nonnull;
import java.util.Random;

public class ItemJudgeStarSlashBlade extends ItemStarSourceSlashBlade {

    public ItemJudgeStarSlashBlade() {
        super("judge_star");
        ItemStack itemStack = new ItemStack(this);
        NBTTagCompound tag = new NBTTagCompound();
        itemStack.setTagCompound(tag);
        itemStack.addEnchantment(Enchantments.INFINITY, 50);
        CurrentItemName.set(tag, "judge_star");
        IsDefaultBewitched.set(tag, true);
        BaseAttackModifier.set(tag, 15f);
        AttackAmplifier.set(tag, 50f);
        KillCount.set(tag, 1);
        ProudSoul.set(tag, 1);
        RepairCount.set(tag, 1);
        SummonedSwordColor.set(tag, new Random().nextInt(256));
        SlashBlade.registerCustomItemStack("judge_star", itemStack);
    ...
    private static ResourceLocationRaw texture = new ResourceLocationRaw("flammpfeil.slashblade", "model/named/starsource/judge_star.png");
    ...
    public void onUpdate(ItemStack sitem, World world, Entity entity, int indexOfMainSlot, boolean isCurrent) {
        ...
        if (!sitem.isItemEnchanted()) {
            sitem.addEnchantment(Enchantments.INFINITY, 50);
        }
    }
    ...
    public SpecialAttackBase getSpecialAttack(ItemStack stack) {
        return JudgeStarSpecialAttacks.theSa;
    }
}
