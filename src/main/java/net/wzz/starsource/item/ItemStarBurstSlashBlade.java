...
import net.wzz.starsource.load.specialAttacks.StarBurstSpecialAttacks;

import javax.annotation.Nonnull;
import java.util.Random;

public class ItemStarBurstSlashBlade extends ItemStarSourceSlashBlade {

    public ItemStarBurstSlashBlade() {
        super("star_burst");
        ItemStack itemStack = new ItemStack(this);
        NBTTagCompound tag = new NBTTagCompound();
        itemStack.setTagCompound(tag);
        itemStack.addEnchantment(Enchantments.INFINITY, 10);
        CurrentItemName.set(tag, "star_burst");
        IsDefaultBewitched.set(tag, true);
        BaseAttackModifier.set(tag, 15f);
        AttackAmplifier.set(tag, 10f);
        KillCount.set(tag, 1);
        ProudSoul.set(tag, 1);
        RepairCount.set(tag, 1);
        SummonedSwordColor.set(tag, new Random().nextInt(256));
        SlashBlade.registerCustomItemStack("star_burst", itemStack);
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        ...
        double speed = 2;
        double dx = -Math.sin(Math.toRadians(yaw)) * Math.cos(Math.toRadians(pitch)) * speed;
        double dy = -Math.sin(Math.toRadians(pitch)) * speed;
        double dz = Math.cos(Math.toRadians(yaw)) * Math.cos(Math.toRadians(pitch)) * speed;
        entityLiving.motionX = dx;
        entityLiving.motionY = dy;
        entityLiving.motionZ = dz;
    }

    private static ResourceLocationRaw texture = new ResourceLocationRaw("flammpfeil.slashblade", "model/named/starsource/star_burst.png");
    ...
    public SpecialAttackBase getSpecialAttack(ItemStack stack) {
        return StarBurstSpecialAttacks.theSa;
    }
}
